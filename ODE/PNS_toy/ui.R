...
fluidPage(titlePanel("Diatom P vs F1 toy model"),
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput(inputId = "dfac",                label = "P cell removal rate due to gametogenesis", step = 0.01, min = 0, max = 1,    value = 0.5),
      numericInput(inputId = "end_time",            label = "Simulation time length",                   step = 1,    min = 1, max = 1000, value = 10),
      numericInput(inputId = "growth_block_start",  label = "Starting day of growth block",             step = 1,    min = 1, max = 9,    value = 2),
      numericInput(inputId = "growth_block_length", label = "Length of the growth block",               step = 1,    min = 1, max = 1000, value = 3),
      numericInput(inputId = "f1_injection_step",   label = "Day of F1 individuals appearence",         step = 1,    min = 1, max = 10,   value = 3),
      numericInput(inputId = "growth_rate_p",       label = "P basal growth rate",                      step = 0.01, min = 0, max = 1,    value = 1.06),
      numericInput(inputId = "growth_rate_f1",      label = "F1 growth rate",                           step = 0.01, min = 0, max = 1,    value = 0.58),
      ...
      numericInput(inputId = "initlen_f",           label = "Starting length of F1 cells",              step = 0.1,  min = 1, max = 100,  value = 55),
